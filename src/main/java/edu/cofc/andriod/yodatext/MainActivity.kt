...
//            }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.root.setBackgroundColor(Color.parseColor("#322214"))

        val editText = findViewById<EditText>(R.id.userTxt)
        editText.requestFocus()
        //focus for keyboard
        window.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_STATE_ALWAYS_VISIBLE)

        binding.sendButton.setOnClickListener {
            val englishText = binding.userTxt.text.toString()
            thread {
                val yodaStr = getYodaString(englishText)
                binding.yodaTxt.post {
                    binding.yodaTxt.text = yodaStr
                }
            }
        }
    }
